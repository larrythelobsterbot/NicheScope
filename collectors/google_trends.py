...
from config import DB_PATH, get_active_keywords, get_keywords_due_for_collection
...
def collect_trends():
    """Collect Google Trends data for active keywords that are due today.

    Keywords are selected and ordered by lifecycle (rising keywords daily,
    stable weekly, declining monthly) so the rate-limit budget is spent on the
    keywords that matter — and so a partial run, if Google starts throttling,
    has already covered the most important ones.
    """
    pytrends = TrendReq(hl="en-US", tz=480)
    db = get_db()
    cursor = db.cursor()

    # Priority-ordered list of (keyword, category) due for a refresh today.
    due = get_keywords_due_for_collection()
    kw_to_cat = {kw: cat for kw, cat in due}
    # All active keyword names — used to avoid re-suggesting tracked keywords.
    existing_lower = {
        kw.lower() for kws in get_active_keywords().values() for kw in kws
    }
    total_collected = 0

    remaining = GOOGLE_TRENDS.get_remaining_today()
    if remaining < len(due):
        logger.warning(
            f"Rate limit: {remaining} requests remaining today, {len(due)} keywords due"
        )
    logger.info(f"{len(due)} active keywords due for collection (lifecycle-weighted)")
        ...
        try:
            ...
            try:
                ...
                for keyword in batch:
                    if keyword in related and related[keyword]["rising"] is not None:
                        ...
                        category = kw_to_cat.get(keyword, "general")
                        for rising_kw in rising_list:
                            if rising_kw.lower() not in existing_lower:
            ...
            try:
                ...
                if not region_df.empty:
                    kw = batch[0]
                    top_regions = (
                        region_df[kw].sort_values(ascending=False).head(20).to_dict()
                    )
